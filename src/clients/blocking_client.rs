...
impl BlockingClient {
    ...
    /// `SocketAddr`. This includes `SocketAddr` and strings. The
    /// `ToSocketAddrs` trait is the Tokio version and not the `std`
    /// version.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// use db_rs::clients::BlockingClient;
    ///
    /// fn main() {
...
impl BlockingSubscriber {
    ...
    /// Convert the subscriber into an `Iterator` yielding new messages
